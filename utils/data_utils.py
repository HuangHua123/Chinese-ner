...
def iob2(tags):
    ...
    for i, tag in enumerate(tags):
        ...
        if split[0] == 'B':
            continue
        elif i == 0 or tags[i - 1] == 'O':  # 将IOB1转换为IOB2
            tags[i] = 'B' + tag[1:]
...
def load_word2vec(emb_path, id_to_word, word_dim, old_weights, logger):
    """
        从预先训练的文件加载单词嵌入
        嵌入大小必须匹配
    ...
    for i, line in enumerate(codecs.open(emb_path, 'r', 'utf-8')):
        ...
        if len(line) == word_dim + 1:
            ...
            emb_invalid += 1
    if emb_invalid > 0:
        logger.info('WARNING: %i invalid lines' % emb_invalid)
    c_found = 0
    c_lower = 0
    c_zeros = 0
    ...
    logger.info('%i / %i (%.4f%%) words have been initialized with '
...
def full_to_half(s):
    ...
    for char in s:
        num = ord(char)
        if num == 0x3000:      #判断是否为全角字符   0x20  半角
            num = 32
